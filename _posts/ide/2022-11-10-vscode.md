...
- *ms-vscode.cpptools* (C/C++), *ms-vscode*, *cpptools-extension-pack* (various tools for C/C++), *twxs.cmake* (CMake language support), *ms-vscode.cmake-tools* (CMake integration), *ms-azuretools.vscode-docker* (Docker), *cschlosser.doxdocgen* (Doxygen), hediet.vscode-drawio (draw.io), mhutchie.git-graph (git graph), ms-toolsai.jupyter (jupyter), mathematic.vscode-latex (latex), James-Yu.latex-workshop (latex), ms-python.python (python), ms-vscode-remote.remote-ssh, tomoki1207.pdf (pdf), DavidAnson.vscode-markdownlint (markdown)
- easier code: steoates.autoimport,  <!-- austin.code-gnu-global (C++ Intellisense) -->
...
- `.vscode/settings.json`: used to configure various **settings**, such as *editor* preferences, *language-specific* settings, and *extensions*. <!-- You can create and edit this file by clicking on the "Settings" icon on the left-hand side (it looks like a gear), and then clicking the "Open Settings (JSON)" button. -->
...
- Run & Debug `launch.json` <!-- <https://code.visualstudio.com/docs/python/debugging> -->
...
- [IntelliSense](https://code.visualstudio.com/docs/python/editing#_autocomplete-and-intellisense): Edit your code with auto-completion, code navigation, syntax checking and more. `Ctrl+Space` to trigger IntelliSense (austin.code-gnu-global)
...
[Docker inside VS Code](../docker-inside-vs-code.html#run-and-debug-python-within-a-docker-container-in-vs-code)
