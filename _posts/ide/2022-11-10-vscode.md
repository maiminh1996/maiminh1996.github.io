...
```bash
wget https://maiminh1996.github.io/assets/scripts/my_vscode_extensions.sh
...
- `.vscode/settings.json`: used to configure various **settings**, such as *editor* preferences, *language-specific* settings, and *extensions*. Check out [`my_settings.json`](https://maiminh1996.github.io/assets/scripts/my_settings.json). <!-- You can create and edit this file by clicking on the "Settings" icon on the left-hand side (it looks like a gear), and then clicking the "Open Settings (JSON)" button. -->
...
In the `launch.json` file above, we define the python env by setting the `pythonPath` variable, but how to use the python env in a docker container for run/ debug? See this post [Docker inside VS Code](../../2023/docker-inside-vs-code/#run-and-debug-python-within-a-docker-container-in-vs-code).
...
- [Python testing with pytest in VS Code](../../2023/python-testing-with-pytest)
<!-- - [Multi-target debugging in VS Code](../2023/multi-target-debugging-in-vs-code) -->
